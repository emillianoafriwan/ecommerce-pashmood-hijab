...
<head>
    ...
    <title>Detail Pre-Order #{{ $order->id }}</title>
...
<body class="bg-gray-100 min-h-screen p-8">
    ...
    <div class="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-sm">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Detail Pre-Order #{{ $order->id }}</h2>
        ...
        <table class="w-full text-left mb-8">
            ...
            <tfoot>
                <tr>
                    <td colspan="3" class="py-4 font-bold text-right text-lg">Total Pre-Order:</td>
        ...
        <div class="mb-8 p-6 bg-gray-50 rounded-xl border border-gray-200">
            <h3 class="font-bold text-lg mb-4 text-gray-800">📦 Informasi Pengiriman Pre-Order</h3>
        ...
        @if(in_array($order->status, ['pre_order', 'pending']))
           ...
           @php
            ...
            <div class="bg-indigo-50 border border-indigo-200 p-6 rounded-xl mb-6 shadow-sm">
                <h3 class="font-bold text-indigo-800 mb-2">💳 Instruksi Pembayaran Pre-Order</h3>
                <p class="text-indigo-700 mb-4">Silakan transfer DP/lunas sesuai nominal <strong>Rp {{ number_format($order->total_price, 0, ',', '.') }}</strong> agar slot pre-order pashmina Anda diproses.</p>
            ...
            @if($order->rejection_reason)
                <div class="bg-red-50 border border-red-200 p-4 rounded-lg mb-6">
                    <p class="text-red-700 font-bold mb-1">⚠️ Pembayaran Pre-Order Ditolak</p>
            ...
            <form action="{{ route('order.confirm', $order->id) }}" method="POST" enctype="multipart/form-data" class="bg-white border p-6 rounded-xl shadow-sm">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Upload Bukti Transfer Pre-Order:</label>
                ...
                <button type="submit" class="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition shadow-md">
                    Konfirmasi Pre-Order & Kirim Bukti
                </button>
            </form>

        @elseif($order->status == 'waiting')
            <div class="bg-blue-50 p-6 rounded-lg text-center border border-blue-200">
                <p class="text-blue-700 font-bold mb-4">Bukti transfer pre-order terkirim! Menunggu verifikasi Admin...</p>
        ...
        @else
            ...
            <div class="bg-green-50 p-6 rounded-lg text-center border border-green-200">
                <p class="text-green-700 font-bold mb-4">✅ Pre-order sudah dibayar dan diverifikasi!</p>
            ...
            <div class="bg-green-50 border border-green-200 p-6 rounded-xl mt-6 shadow-sm flex flex-col md:flex-row items-center justify-between">
                <div>
                    <h3 class="text-lg font-bold text-green-800 flex items-center gap-2">
                        📦 Pashmina Anda Sedang Dikirim!
                    </h3>
                    <p class="text-sm text-green-700 mt-1">Pre-order Anda dikirim menggunakan kurir <strong>{{ $order->courier }}</strong>.</p>
                </div>
        ...
        @if($order->status == 'shipped')
            <div class="mt-8 bg-blue-50 border border-blue-200 p-6 rounded-xl text-center shadow-sm">
                <h3 class="text-lg font-bold text-blue-800 mb-2">Apakah pashmina sudah Anda terima?</h3>
