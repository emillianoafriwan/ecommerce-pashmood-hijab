...
<head>
    ...
    <title>Admin - Detail Pre-Order #{{ $order->id }}</title>
...
<body class="bg-gray-100 min-h-screen p-8">
    ...
    <div class="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-sm">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Admin: Detail Pre-Order #{{ $order->id }}</h2>
            ...
            @if(!$hasResi)
                ...
                <form action="{{ route('admin.order.generate_resi', $order->id) }}" method="POST">
                    ...
                    <div class="flex flex-wrap gap-3 border-t border-indigo-200 pt-4 mt-4">
                        ...
                        <span class="text-sm text-indigo-700 font-medium flex items-center">
                            Setelah resi dibuat, tombol cetak label dan kirim pre-order akan muncul.
                        </span>
                    </div>
                </form>
            @else
                ...
                <div class="flex flex-wrap gap-3 border-t border-indigo-200 pt-4 mt-4 items-center">
                    ...
                    <!-- Tombol Cetak Resi -->
                    <a href="{{ route('admin.order.print_resi', $order->id) }}" target="_blank" class="bg-blue-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-blue-700 transition shadow-md text-center flex items-center justify-center gap-2 w-full md:w-auto">
                        🖨️ Cetak Label
                    </a>

                    <!-- Tombol Kirim Pre-Order -->
                    @if($isShippedOrCompleted)
                        ...
                        <button type="button" disabled class="bg-green-600 text-white px-6 py-3 rounded-lg font-bold cursor-not-allowed w-full md:w-auto shadow-md">
                            📦 Pre-Order Telah Dikirim / Selesai
                        </button>
                    @else
                        <!-- Form Kirim Pre-Order -->
                        <form action="{{ route('admin.order.ship', $order->id) }}" method="POST" class="w-full md:w-auto">
                            @csrf
                            <button type="submit" class="bg-green-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-green-700 transition shadow-md w-full flex items-center justify-center gap-2">
                                📩  Kirim Pre-Order
                            </button>
                        </form>
                    @endif
        ...
        @else
        <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 p-5 rounded-xl mt-6 shadow-sm">
            Pengiriman pre-order akan aktif setelah pembayaran diverifikasi.
        </div>
        @endif
        ...
        @elseif(in_array($order->status, ['paid', 'shipped', 'completed']))
            ...
            <div class="bg-green-100 text-green-700 p-4 rounded-lg text-center font-bold">
                Pembayaran Pre-Order Sudah Dikonfirmasi.
            </div>
        @endif
