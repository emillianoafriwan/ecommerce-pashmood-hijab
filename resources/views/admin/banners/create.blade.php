...
<head>
    ...
    <title>Tambah Banner — PASHMOOD Admin</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type="text"], input[type="url"], input[type="number"], select, textarea {
            ...
            background: var(--body-bg); transition: border-color .2s, box-shadow .2s;
            outline: none;
        }
        ...
        .upload-icon { width: 48px; height: 48px; color: var(--primary); margin: 0 auto 12px; }
        .upload-area h4 { font-size: 15px; font-weight: 600; margin-bottom: 4px; }
        .upload-area p { font-size: 13px; color: var(--text-soft); }
        #imagePreviewWrapper { margin-top: 16px; display: none; }
        #imagePreview {
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="topbar-title">➕ Tambah Banner Baru</div>
            ...
            <div class="form-card">
                <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label>Gambar Banner <span class="req">*</span></label>
                        <div class="upload-area" id="uploadArea">
                            <input type="file" id="imageInput" accept="image/jpg,image/jpeg,image/png,image/webp" required>
                        ...
                        <input type="file" name="image" id="image-cropped" style="display:none" required>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="mis. Voal Premium" required maxlength="100">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', 0) }}" min="0" max="99" required>
                            <div class="hint">Angka lebih kecil tampil lebih dulu (0 = pertama)</div>
                        ...
                        <div class="form-group full">
                            ...
                            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle') }}" placeholder="mis. Tekstur lembut, tegak di dahi." maxlength="200">
                        ...
                        <div class="form-group full">
                            ...
                            <input type="url" name="link_url" id="link_url" value="{{ old('link_url') }}" placeholder="https://..." maxlength="255">
                            <div class="hint">URL tujuan saat banner diklik. Biarkan kosong jika tidak ada link.</div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="placement" id="placement" required>
                                <option value="below_recommendations" {{ old('placement') == 'below_recommendations' || request('placement') == 'below_recommendations' ? 'selected' : '' }}>Di bawah Rekomendasi (Tengah - Default)</option>
                                <option value="above_recommendations" {{ old('placement') == 'above_recommendations' || request('placement') == 'above_recommendations' ? 'selected' : '' }}>Di atas Rekomendasi</option>
                                <option value="below_catalog" {{ old('placement') == 'below_catalog' || request('placement') == 'below_catalog' ? 'selected' : '' }}>Di bawah Katalog Utama</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="width" id="width" required>
                                <option value="half" {{ old('width') == 'half' ? 'selected' : '' }}>Setengah Lebar (2 Kolom)</option>
                                <option value="full" {{ old('width') == 'full' ? 'selected' : '' }}>Lebar Penuh (1 Kolom)</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="height" id="height" required>
                                <option value="medium" {{ old('height') == 'medium' ? 'selected' : '' }}>Sedang (Default)</option>
                                <option value="small" {{ old('height') == 'small' ? 'selected' : '' }}>Pendek</option>
                                <option value="large" {{ old('height') == 'large' ? 'selected' : '' }}>Tinggi</option>
                            </select>
...
(function () {
    ...
    function applyCrop() {
        ...
        canvas.toBlob(function (blob) {
            ...
            previewLabel.textContent = `File cropped: ${sizeStr}`;
            
            previewWrapper.style.display = 'block';
            recropBtn.style.display = 'inline-block';
            
            cropHint.textContent = '✓ Gambar siap diunggah.';
    ...
    if (bannerForm) {
        bannerForm.addEventListener('submit', function (e) {
            if (croppedInput && !croppedInput.files.length) {
                e.preventDefault();
                cropHint.textContent = 'Pilih gambar banner lalu crop terlebih dahulu.';
                cropHint.style.color = '#ef233c';
                imageInput?.focus();
            }
        });
