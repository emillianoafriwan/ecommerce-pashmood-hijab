...
<body class="bg-[#F8FAFC] text-slate-900 min-h-screen pb-20">
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div class="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            ...
            <form id="product-form" action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" data-no-ajax="true">
                @csrf 
                @method('PUT')
                ...
                <div class="mb-8">
                    ...
                    <textarea name="description" id="description" rows="4" 
                        class="w-full px-5 py-4 rounded-2xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-rose-500 transition outline-none text-slate-800 font-bold bg-slate-50 focus:bg-white placeholder-slate-300"
                        placeholder="Pashmina premium dengan material pilihan yang memberikan kenyamanan maksimal. Tekstur lembut, mudah dibentuk, dan memberikan kesan mewah pada penampilan Anda.">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="text-rose-500 text-xs mt-2 font-bold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-8 p-5 md:p-6 bg-rose-50/60 border border-rose-100 rounded-[2rem]">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
                        ...
                        <button type="button" onclick="addVariationRow()" class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-rose-600 transition gap-2 w-full sm:w-auto">
                        ...
                        @foreach($product->variations as $index => $variation)
                            <div class="variation-row grid grid-cols-1 md:grid-cols-[2fr_3fr_auto] gap-3 bg-white p-4 rounded-2xl border border-rose-100 shadow-sm items-center">
                                ...
                                <input type="text" name="variations[{{ $index }}][color]" value="{{ $variation->color }}" 
                                    placeholder="Warna (contoh: Mocca)" class="w-full px-4 py-3 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-rose-500 outline-none text-sm font-bold text-slate-700 bg-slate-50 focus:bg-white">
                                <div class="flex items-center gap-3">
                                    @if($variation->image_path)
                                        <img src="{{ asset('storage/' . $variation->image_path) }}" class="w-12 h-12 object-cover rounded-xl border border-rose-100 shadow-sm shrink-0">
                                    @endif
                                    <div class="flex-1 flex items-center gap-2">
                                        <span class="text-xs font-bold text-slate-500 shrink-0">Ganti Foto:</span>
                                        <input type="file" name="variations[{{ $index }}][image]" accept="image/*" class="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-rose-50 file:text-rose-700 hover:file:bg-rose-100 cursor-pointer">
                                    </div>
                                </div>
                                <button type="button" class="h-12 w-12 inline-flex items-center justify-center text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition" onclick="removeVariationRow(this)" title="Hapus Warna">
                ...
                <div class="mb-10 p-5 md:p-6 bg-slate-50 border border-slate-100 rounded-[2rem]">
                    ...
                    {{-- File picker (visible, triggers crop modal) --}}
                    ...
                    {{-- Preview box (shows existing image or cropped result) --}}
                    <div id="img-preview-box" @if(!$product->image_path) style="display:none" @endif>
                        <img id="img-preview" src="{{ $product->image_path ? asset('storage/' . $product->image_path) : '' }}" alt="Preview Gambar">
                        <button type="button" id="recrop-btn" class="recrop-btn" style="{{ $product->image_path ? '' : 'display:none' }}" onclick="reopenCrop()">
                            ✂ Crop Ulang
                        </button>
                    </div>

                    <p id="crop-hint" class="text-xs text-slate-400 mt-2 font-medium">
                        @if($product->image_path)
                            Gambar saat ini tersimpan di storage. Unggah file baru hanya jika ingin menggantinya.
                        @else
                            Pilih gambar lalu crop sesuai kebutuhan.
                        @endif
                    </p>
    ...
    <script>
        ...
        /* ── Variation rows ── */
        var varRowCount = {{ $product->variations->count() }};
        ...
        function addVariationRow() {
            ...
            newRow.innerHTML = `
                <input type="text" name="variations[${varRowCount}][color]" placeholder="Warna (contoh: Mocca)" class="w-full px-4 py-3 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-rose-500 outline-none text-sm font-bold text-slate-700 bg-slate-50 focus:bg-white">
                <div class="flex items-center gap-2">
                    <span class="text-xs font-bold text-slate-500 shrink-0">Foto Warna:</span>
                    <input type="file" name="variations[${varRowCount}][image]" accept="image/*" class="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-rose-50 file:text-rose-700 hover:file:bg-rose-100 cursor-pointer">
                </div>
                <button type="button" class="h-12 w-12 inline-flex items-center justify-center text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition" onclick="removeVariationRow(this)" title="Hapus Warna">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
        ...
        function removeVariationRow(btn) {
            ...
            if (rows.length > 1) {
                ...
                alert('Produk harus memiliki minimal 1 variasi warna.');
            }
        }
        ...
        function reopenCrop() {
            if (originalFile) {
                ...
                // No file picked yet, trigger picker
                picker.click();
            }
        }
        ...
        function applyCrop() {
            ...
            canvas.toBlob(function (blob) {
                ...
                cropHint.textContent = '✓ Gambar baru siap diunggah. Klik "Crop Ulang" jika ingin mengubah.';
        ...
        productForm.addEventListener('submit', function (e) {
            if (originalFile && !croppedInput.files.length) {
                e.preventDefault();
                cropHint.textContent = 'Klik "Gunakan Gambar Ini" setelah crop, atau pilih gambar ulang.';
                cropHint.style.color = '#e11d48';
                picker.focus();
            }
        });
